...
pub struct ConfigurationInner {
    ...
    /// What kind of cache/persistent store to use. Supported: memory, redis (must have redis_dsn or
    /// cache_dsn configured), rediscluster (must also have a DSN set) and none.
    pub cache_type: CacheType,
    /// The DSN for the Redis-backed cache/persistent store. Overrides `redis_dsn`. (can be left
    /// empty if not using redis)
...
impl ConfigurationInner {
    ...
    pub fn shared_store_backend(&self) -> SharedStoreBackend<'_> {
        ...
        match self.shared_store_type() {
            SharedStoreType::None => SharedStoreBackend::None,
    ...
    pub fn shared_store_type(&self) -> SharedStoreType {
        match self.cache_type {
            CacheType::None => SharedStoreType::None,
